...
# %% auto 0
__all__ = ['RNNWrapper', 'RNNWrapperHS', 'BaseApproximator', 'BaseApproximatorMLP', 'RNNMLPHybrid', 'RL2RNN',
           'BaseApproximatorRNN', 'BaseApproximatorRL2RNN', 'MLPStateAction', 'MLPState', 'MLPActor', 'RNNActor',
           'RNNStateAction', 'RL2RNNActor', 'RL2RNNValue']
...
class RL2RNN(nn.Module, BaseApproximator):
    ...
    def forward(self, x, hidden_state=None):
        """
        Forward pass through the RNN + optional MLP.
        Args:
        ...
        if x.dim() == 2:
            x = x.unsqueeze(1)
        rnn_out, hidden_state = self.rnn_core(x, hidden_state)
        if rnn_out.dim() == 3:
            last_out = rnn_out[:, -1, :]
        elif rnn_out.dim() == 2:
            last_out = rnn_out
        else:
            raise ValueError(f"Unexpected rnn_out shape: {rnn_out.shape}")
...
# %% ../nbs/30_agents/60_approximators/21_critic_networks.ipynb 18
