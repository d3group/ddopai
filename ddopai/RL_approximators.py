...
# %% auto 0
__all__ = ['RNNWrapper', 'BaseApproximator', 'BaseApproximatorMLP', 'RNNMLPHybrid', 'RL2RNN', 'BaseApproximatorRNN',
           'BaseApproximatorRL2RNN', 'MLPStateAction', 'MLPState', 'MLPActor', 'RNNActor', 'RNNStateAction',
           'RL2RNNActor', 'RL2RNNValue']
...
class RL2RNN(nn.Module, BaseApproximator):
    ...
    def __init__(self, RNN_input_size: int,
        ...
        self.hidden_dim = num_hidden_units_RNN  # << ADDED
        self.rnn_cell_type = RNN_cell.lower()    # << ADDED
        # RNN
        # RNN layers
        ...
        if final_activation.lower() != "identity":
            layers_MLP.append(FinalActivation())

        self.main_mlp = nn.Sequential(*layers_MLP)
        
    def forward(self, x, hidden_state=None):
        """
        Forward pass through the RNN and optional MLP.
        - x: [batch_size, seq_len, input_dim]
        - hidden_state: RNN hidden state
        """
        rnn_out, hidden_state = self.rnn(x, hidden_state)
        last_out = rnn_out[:, -1, :]  # Use last timestep output

        output = self.main_mlp(last_out)
    ...
    def init_hidden(self, batch_size=1, device='cpu'):
        ...
        """
        shape = (self.rnn.num_layers, batch_size, self.hidden_dim)
...
# %% ../nbs/30_agents/60_approximators/21_critic_networks.ipynb 14
...
# %% ../nbs/30_agents/60_approximators/21_critic_networks.ipynb 15
...
# %% ../nbs/30_agents/60_approximators/21_critic_networks.ipynb 16
class RL2RNNActor(BaseApproximatorRL2RNN):
    ...
    def forward(self, state, hidden_state=None):
        ...
        """
        mean, hidden_state = self.model(state, hidden_state)
...
# %% ../nbs/30_agents/60_approximators/21_critic_networks.ipynb 17
class RL2RNNValue(BaseApproximatorRL2RNN):
    ...
    def forward(self, state, hidden_state=None):
        ...
        """
        value, hidden_state = self.model(state, hidden_state)
        value = value.squeeze(-1)  # ensure scalar output
