...
class MultiShapeLoader(BaseDataLoader):
    ...
    def __getitem__(self, idx: int):
        ...
        if include_y:
            ...
            lag_demand = self.demand_lag[idx_time-1-lag_window:idx_time, idx_skus] # need to use t-1 to get the lag  
        ...
        time_features = self.time_features[idx_time-lag_window:idx_time+1]
        ...
        for i, idx_sku in enumerate(idx_skus):
            ...
            time_SKU_features = self.time_SKU_features[idx_time-lag_window:idx_time+1, SKU_indices]
            item[:,:,(len_SKU_features+len_time_features):(len_SKU_features+len_time_features+num_time_SKU_features_without_lag_demand),i] = np.expand_dims(time_SKU_features, axis=0)
        ...
        if include_y:
            item[:,:,-1,:] =  np.expand_dims(lag_demand, axis=0)
            if self.include_non_available:
                item[:,:,-2,:] = np.expand_dims(self.mask[idx_time-lag_window:idx_time+1,idx_skus],axis=0)
            
        else: 
            if self.include_non_available:
                item[:,:,-1,:] = np.expand_dims(self.mask[idx_time-lag_window:idx_time+1,idx_skus],axis=0)
